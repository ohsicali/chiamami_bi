/* ============================================================================
   LA HOME DEL COMPUTER — quella di prima del rifacimento v10.

   Ci sono due home, e sono due file veri, non due rami dentro lo stesso
   componente. Il motivo è che non condividono la struttura: qui si apre col
   drop in evidenza e il momento scende sotto, nella v10 (HomeFeedV4, il
   telefono) si apre con l'orologio e la fascia del momento. Non è una
   differenza che si fa spostando dei margini: è un altro ordine di blocchi,
   con altri componenti dentro. Tenere tutt'e due dentro un file solo avrebbe
   voluto dire due alberi JSX completi uno sopra l'altro, montati entrambi e
   nascosti a vicenda col CSS — doppio lavoro al browser su ogni pagina.

   Quale delle due si vede lo decide App.jsx con una media query a 1024px,
   e ognuna arriva col suo `lazy`: chi apre il sito dal telefono non scarica
   nemmeno il codice di questa.

   IL PREZZO, detto chiaro: d'ora in poi una modifica alla home va fatta
   due volte, qui e in HomeFeedV4. Vale anche per i due componenti gemelli,
   TimeContextHeroClassic e MomentResultsGridClassic, che esistono per la
   stessa ragione. Se un domani una delle due versioni viene abbandonata,
   questi file vanno cancellati, non lasciati lì.
   ========================================================================= */
...
import TimeContextHero from '../../components/Home/TimeContextHeroClassic'
import MomentTabs from '../../components/Home/MomentTabs'
import MomentResultsGrid from '../../components/Home/MomentResultsGridClassic'
...
export default function HomeDesktopClassic() {
  const navigate = useNavigate()
  const { user } = useAuth()
  ...
  // La porta a vetri: si apre sopra la home invece di portare via, così chi
  // decide di non registrarsi resta dov'era.
  ...
  // "Sblocca sconto" faceva esattamente la stessa cosa di "Scopri": apriva la
  // scheda del locale. Un bottone col lucchetto che non sblocca niente è una
  // promessa mancata, e sulla scheda poteva pure comparire un ALTRO sconto
  // dello stesso locale al posto del drop da cui si era partiti — si tocca
  // 50% e si legge 1€. Da sloggati si apre il gate, da loggati si va nel Bi
  ...
  // utente, e qui nessuno controllava prima di chiamarlo. Il tocco spariva,
  // senza cuore pieno né spiegazione.
  ...
  // Al rientro dalla registrazione il locale si salva da solo: chi ha toccato
  // il cuore aveva già detto cosa voleva.
  useEffect(() => {
    if (!user?.id) return
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        /* La barra con logo e "Chiedi a Bi" è quella del telefono: la navbar
           vera (DesktopNavbar) compare già da 768px con md:block, e finché
           questa spariva solo a 1024 su un iPad in verticale si vedevano due
           intestazioni una sopra l'altra. */
